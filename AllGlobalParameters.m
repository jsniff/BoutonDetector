...
global CellNumberofImages;
    ...
    %CellThresholdParameter = .28;
    %CellconnectivitySize = 800;
...
global GaussianSigma;
    ...
    BoutonThresholdParameter = .16;
     ...
     cd('/Users/jsniff/Desktop/PVProject/PVSynapseProjectAllFiles/MATLABFILES/');


    BoutonListDirectory = '/Users/jsniff/Desktop/PVProject/PVSynapseProjectAllFiles/MATLABFILES/BoutonsDetected_lists_tests';

% 
%GaussianFilterRadius = 10;
% e = exec(conn,['create table IF NOT EXISTS basketneurons.luke (lukeindex varchar(20) NOT NULL DEFAULT '''' PRIMARY KEY, '...
...
%    ' ''lukeboutons.reformatted.txt'' into table basketneurons.luke '...
...
%GaussianFilterRadiusVector  = [5:1:10];
  %GaussianFilterRadius = GaussianFilterRadiusVector(i);
    %GaussianFilterRadius = 7.5;

%AcceptanceCellDistanceVector  = [15:1:25];
...
for i = 1:length(ConnectivitySizeVector)
  ...
  UniqueCode=cell2mat(resultssqlqueryuniquecode)+1;
    ...
    %AcceptanceCellDistance = AcceptanceCellDistanceVector(i);
  ...
  CellThresholdParameter = CellThresholdParameterVector(j);
    ...
    FinalName = strcat(BoutonListDirectory,'/test_',num2str(UniqueCode));
    %keyboard;
...
% e = exec(conn,['load data local infile '...
%    ' ''boutonrevised.txt'' into table basketneurons.jacob '...
%    'fields terminated by ''\t'' lines terminated '...
%    'by ''\n''']);


%execute this code to create All Test Parameters Code
% 
%  e = exec(conn,['create table IF NOT EXISTS basketneurons.AllTestParameters (uniquecode int NOT NULL PRIMARY KEY, '...
%    'testname varchar(20), GaussianFilterRadius smallint NOT NULL, GaussianFilterSigma smallint NOT NULL, ConnectivitySize smallint NOT NULL, '...
%    'CellThresholdParameter FLOAT NOT NULL, BoutonThresholdParameter FLOAT NOT NULL, DoubleCountDistance smallint NOT NULL,  AcceptanceCellDistance smallint NOT NULL,  Precision_Cell1 FLOAT NOT NULL, Recall_Cell1 FLOAT NOT NULL, Precision_Cell2 FLOAT, Recall_Cell2 FLOAT)']);
%     close(e);
%   
...
%          e = exec(conn,['create table IF NOT EXISTS basketneurons.AllTestParameters (uniquecode int NOT NULL PRIMARY KEY, '...
%     'testname varchar(20), GaussianFilterRadius smallint NOT NULL, GaussianFilterSigma smallint NOT NULL, ConnectivitySize smallint NOT NULL, '...
%      'CellThresholdParameter FLOAT NOT NULL, DoubleCountDistance smallint NOT NULL, AcceptanceCellDistance smallint NOT NULL, BoutonThresholdParameter FLOAT NOT NULL, CellconnectivitySize int NOT NULL, CellSizeDistance smallint NOT NULL, CellNumberofImages smallint NOT NULL, CellSizeThreshold int, Precision_Cell1 FLOAT NOT NULL, Recall_Cell1 FLOAT NOT NULL, Precision_Cell2 FLOAT, Recall_Cell2 FLOAT)']);
%      close(e);
...
% 'where productDescription = ' '''' productdesc '''']; 
 ...
 %bouton_num_jacob_cell_4 = results1_mat(4);

 
  %  e = exec(conn,sqlquery1);
  % close(e);
...
%  
%  ParameterFile = 'AllTestParameters';
%  fid=fopen(ParameterFile,'a');
%  fprintf(fid, '%s\t%i\t%i\t%.2f\t%.2f\t%i\t%i\t%.2f\t%.2f\n', FinalName, GaussianFilterRadius,GaussianSigma,ConnectivitySize,CellThresholdParameter,BoutonThresholdParameter,DoubleCountDistance,AcceptanceCellDistance,precision_cell_1,recall_cell_1);
%  fclose(fid);
%  
%  
